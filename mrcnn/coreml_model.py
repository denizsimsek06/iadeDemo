...
class ProposalLayer(KE.Layer):
    ...
    def __init__(self, proposal_count, nms_threshold, images_per_gpu, rpn_bbox_std_dev, name, **kwargs):
        self.name = name
        # self.config = config
...
def refine_detections_graph(rois, probs, deltas, window, bbox_std_dev, detection_min_confidence,
...
class DetectionLayer(KE.Layer):
    ...
    def __init__(self, batch_size, images_per_gpu, bbox_std_dev, detection_min_confidence, detection_max_instances,
    ...
    def call(self, inputs):
        ...
        detections_batch = utils.batch_slice(
            ...
            lambda x, y, w, z: refine_detections_graph(x, y, w, z, self.bbox_std_dev, self.detection_min_confidence,
    ...
    def get_config(self):
        ...
        mconfig = {'batch_size': self.batch_size,
        ...
        all_config = dict(list(base_config.items()) + list(mconfig.items()))
        return all_config
...
############################################################
# workaround for NameError: name 'tf' is not defined error within KL.Lambda calls
def lambda_a(t):
    import tensorflow as tf
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    rpn_class_logits = KL.Lambda(lambda_a)(x)
...
############################################################
def lambda_c(t):
    return K.squeeze(K.squeeze(t, 3), 2)
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    assert gt_class_ids.dtype == np.int32 or np.int64, "Expected int but got {}".format(
        gt_class_ids.dtype)
    assert gt_boxes.dtype == np.int32 or np.int64, "Expected int but got {}".format(
        gt_boxes.dtype)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        input_image = KL.Input(
            shape=[val for val in config.IMAGE_SHAPE], name="input_image")
        ...
        detection_boxes = KL.Lambda(lambda_d)(detections)
